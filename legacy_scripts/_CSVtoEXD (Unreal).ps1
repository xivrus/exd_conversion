...
if ($exh_answer.ToLower() -eq 'all') {
    ...
    switch ($_answer) {
        ...
        default {
            ...
            "E.g. the 100th string from 12th file would start from 'C_64_'."
            $_answer = $(Read-Host "Add a file index at the start of translation fields? (y/N)")
            switch ($_answer) {
...
if (!$GLOBAL_CUSTOMIZE) {
    "$($exh_file_name): Choose what to do with '<index>_' at the start of translation fields:"
    $_answer = $(Read-Host "Add if doesn't exist / Remove if exists / do Nothing (a/r/N)").ToLower()
    switch ($_answer) {
        "a" { $INDEXES_CHOICE = 1; break }
        "r" { $INDEXES_CHOICE = 2; break }
...
# For easier parsing we'll make a single string out of CSV. I'm going to use CP1251
...
do {
    ...
    if ($WRITE_FILE_INDEX_IN_TRANSLATION) {
        ...
        $file_index_byte = [System.Text.Encoding]::ASCII.GetBytes($file_index_hex)
    } else { $file_index_byte = $null }
    ...
    if ($row_translation_length -gt 0) {
        if ( ($INDEXES_CHOICE -eq 1) -and $_add_index_bytes) {
            $translation_bytes = $file_index_byte + $exd_index_byte + $csv[$row_translation_start_index..$row_translation_end_index]
            $_add_index_bytes = $false   # Reset variable
        } else {
            $translation_bytes = $file_index_byte + $csv[$row_translation_start_index..$row_translation_end_index]
        }
        $translation_bytes = Convert-TagsToVariables $translation_bytes
    } else { $translation_bytes = $file_index_byte + @() }
...
if ($WRITE_FILE_INDEX_IN_TRANSLATION) {
    "Folder conversion complete. File indexes:"
